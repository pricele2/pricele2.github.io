...
So how do we determing which API endpoints might we want? 
...
# Might need to fuss with "max print" setting 
# getOption("max.print")
# options(max.print = 50000)
```

The analyst's BFF: `censusapi::listCensusMetadata`

Critical argument is `type`:

- "geographies", or "groups", or "variables" (use include_values = TRUE)

- Can confirm here that Block-Level is for the Decennial (PL or DHC) survey only
...
## Example: Most recent ACS 5-Year

Heads up: This will take a bit to run... so keep it as an object.
```{r}
#| label: list.census.metadata
#| eval: true
vars_acs5 = listCensusMetadata(
  "acs/acs5",
  vintage = 2024,
  key = NULL,
  type = "variables" #  list of variable names and descriptions 
)
```
Once you have a sense of which survey and which data elements are relevant...

## Where are your geographies of interest? 

*"What are some characteristics of our neighborhoods?"*

Requires knowing the census 'address' of the programs!

You can visit [geocoding.geo.census.gov](https://geocoding.geo.census.gov/geocoder/geographies/onelineaddress?form)...
...
We've drawn the circle, so let's see who is inside it. 

Now we have `br_5km`, an input area representing the 5 kilometers around the address. So we can use this as the `filter_by` argument in `get_decennial()` or `get_acs()`.

This gives back the Census block groups within 5km (5000 meters) of the address without any additional need for spatial overlay.
...
# Using the same tidycensus::get_decennial() from earlier
...
## Now let's geocode all the Head Start sites

First we'll read in the CSV, converting it to a dataframe. 

(Your read_csv path should be where you downloaded the file, in slide 2.)
```{r}
...
#| code-overflow: wrap
#| output: false
# Read in the file, wrangle into tibble without rownames
...
colSums(is.na(all_hs)) #missing values ~ probably need data cleaning
```
...
hs_geos = hs_geos |> select(grant_Number, program_type_label, recipient_name, service_location_name, input_address, funded_slots, status, lat, long, ends_with("fips"), starts_with("match"), starts_with("census"), starts_with("tiger"))

```
Consider: Why does it confirm `Passing 983 addresses` instead of 1267?

```{r}
table(hs_geos$match_type)
```
...
# Filter the inherited object for only locations in our 13 counties
hs_geos2 = hs_geos |> 
  filter(state_fips == "48") |>  #drops the rows with "NA" returned
  filter(county_fips %in% ws_gulf_coast) #drops the rowsthat aren't in our list

length(unique(hs_geos2$input_address))
...
There are 117 unique `input_address`es but 152 rows. What do we know about this dataframe that explains this?
...
- Census data can be used to contextualize nearly every program at Baker Ripley, even indirectly. For example: Behavioral Risk Factors data from CDC, [at the tract level](https://www.cdc.gov/places/about/index.html)
